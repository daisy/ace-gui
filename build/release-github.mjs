...
(async () => {
  ...
  try {
    getReleaseByTagRES = await octokit.repos.getReleaseByTag({
      owner,
      repo,
      tag,
    });
    ...
    if (DEBUG)
      console.log(
        (err?.toString ? err.toString() : String(err)).replace(
          ghtoken,
          "GH_TOKEN_XX",
        ),
      );
  ...
  if (getReleaseByTagRES) {
    ...
    if (SAFE_DEBUG)
      console.log(JSON.stringify(getReleaseByTagRES.data, null, 4));

    console.log("################################################");
    console.log("################ listReleaseAssets: " + release_id);
    let listReleaseAssetsRES = undefined;
    try {
      listReleaseAssetsRES = await octokit.repos.listReleaseAssets({
        owner,
        repo,
        release_id,
      });
    ...
    if (listReleaseAssetsRES) {
      ...
      if (SAFE_DEBUG)
        console.log(JSON.stringify(listReleaseAssetsRES.data, null, 4));
      ...
      for (const asset of listReleaseAssetsRES.data) {
        ...
        try {
          deleteReleaseAssetRES = await octokit.repos.deleteReleaseAsset({
            owner,
            repo,
            asset_id,
          });
          ...
          console.log("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
          console.log(
            "deleteReleaseAsset error! Continue ... " +
              release_id +
              " ==> " +
              asset_id,
          );
        ...
        if (deleteReleaseAssetRES) {
          console.log(
            "deleteReleaseAsset OK: " + release_id + " ==> " + asset_id,
          );
          if (DEBUG) console.log(deleteReleaseAssetRES);

          if (SAFE_DEBUG)
            console.log(JSON.stringify(deleteReleaseAssetRES.data, null, 4));
        }
      }
    }
    ...
    try {
      deleteReleaseRES = await octokit.repos.deleteRelease({
        owner,
        repo,
        release_id,
      });
    ...
    if (deleteReleaseRES) {
      ...
      if (SAFE_DEBUG)
        console.log(JSON.stringify(deleteReleaseRES.data, null, 4));
    }
  }
  ...
  if (createTagRES) {
    ...
    try {
      createRefRES = await octokit.git.createRef({
        owner,
        repo,
        ref: ref2,
        sha,
  ...
  console.log("################################################");
  console.log(
    "################ createRelease: " + tag_name + " -- " + target_commitish,
  );
  ...
  if (createReleaseRES && createReleaseRES.data && createReleaseRES.data.id) {
    ...
    const upload = async (filename, filepath) => {
      ...
      try {
        ...
        console.log("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
        console.log(
          "uploadReleaseAsset error! Continue ... " +
            release_id2 +
            " ==> " +
            filename,
        );
      ...
      if (uploadReleaseAssetRES) {
        console.log(
          "uploadReleaseAsset OK: " + release_id2 + " ==> " + filename,
        );
        if (DEBUG) console.log(uploadReleaseAssetRES);

        if (SAFE_DEBUG)
          console.log(JSON.stringify(uploadReleaseAssetRES.data, null, 4));
      }
    };
    ...
    try {
      listReleaseAssetsRES2 = await octokit.repos.listReleaseAssets({
        owner,
        repo,
        release_id: release_id2,
      });
    ...
    if (listReleaseAssetsRES2) {
      ...
      if (SAFE_DEBUG)
        console.log(JSON.stringify(listReleaseAssetsRES2.data, null, 4));
    ...
    if (updateReleaseRES) {
      ...
      if (SAFE_DEBUG)
        console.log(JSON.stringify(updateReleaseRES.data, null, 4));
    }
  }
})(); // top-level ASYNC
