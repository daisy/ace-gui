...
glob(`${srcFolderPath}/**/*{.js,.jsx}`, {}, function (err, files) {
    if (err) {
        console.log(err);
        process.exit(1);
        return;
    ...
    for (const file of files) {
        ...
        // localize\s*\(\s*['"]([^'"]+)['"]
        const regex = new RegExp(`localize\\s*\\(\\s*['"]([^'"]+)['"]`, "g");
    ...
    jsonObj.report.summarySection.other = "";
    jsonObj.report.summarySection["best-practice"] = "";
